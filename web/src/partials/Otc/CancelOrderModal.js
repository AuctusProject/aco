...
class CancelOrderModal extends Component {
  ...
  onCancelOrder = () => {
    ...
    cancel(this.context.web3.selectedAccount, this.props.cancelOrderData.nonce)
    .then((result) => {
      if (result) {
        this.setStepsModalInfo(++stepNumber)
  ...
  setStepsModalInfo = (stepNumber) => {
    ...
    if (stepNumber === 1) {
      ...
      subtitle = "Canceling OTC order..."
      img = <SpinnerLargeIcon />
    }
    else if (stepNumber === 3) {
      subtitle = "You have successfully canceled your OTC order."
      img = <DoneLargeIcon />
    }
...
CancelOrderModal.contextTypes = {
  web3: PropTypes.object
}
export default withRouter(CancelOrderModal)