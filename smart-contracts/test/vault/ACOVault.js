...
describe("ACOVault", function() {
  ...
  const createVaultStrategy = async () => {
    ...
    await crv.connect(owner).transfer(pairCrvToken2.address, ethers.utils.bigNumberify("5000000000000000000000000"));
  ...
  describe("Vault transactions", function () {
    ...
    it("Vault earn", async function () {
      let deposit = ethers.utils.bigNumberify("10000000000");
      ...
      let deposit2 = ethers.utils.bigNumberify("1000000000000");
    ...
    it("Vault exercise", async function () {
      ...
      validateAboveMinValue((await token2.balanceOf(vault.address)).sub(balVault), ethToken2Price.mul(5).mul(acoBal).div(ethers.utils.bigNumberify("100000000000000000000")), 7000);
      ...
      validateAboveMinValue((await token2.balanceOf(vault.address)).sub(balVault), token1Token2Price.mul(5).mul(acoBal.div(2)).div(ethers.utils.bigNumberify("10000000000")), 8000);
      ...
      expect(await aco.balanceOf(await addr3.getAddress())).to.equal(acosAmount3[0]);
      expect(await aco.balanceOf(vault.address)).to.equal(2);
    ...
    it("Vault skim", async function () {
      ...
      expect(await aco2.balanceOf(vault.address)).to.equal(2);
      expect(await vault.balance()).to.equal(1);
  ...
  describe("Vault's complex flows", function () {
    it("Vault flow1", async function () {
      ...
      expect(acos2.length).to.equal(1);
      expect(acos3.length).to.equal(1);
      expect(accountBalance1).to.be.above(depositValue);
      expect(accountBalance2).to.be.above(depositValue);
      expect(accountBalance3).to.be.above(depositValue.div(2));
