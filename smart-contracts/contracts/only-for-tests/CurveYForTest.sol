 ...
 */
contract CurveYForTest is ICurveFi4 {
    uint256 internal constant N_COINS = 4;

    uint256 ZERO256 = 0;
    uint256[N_COINS] internal ZEROS = [ZERO256, ZERO256, ZERO256, ZERO256];

    uint256[N_COINS] internal PRECISION_MUL = [uint256(1), 1000000000000, 1000000000000, 1];
    uint256 internal constant PRECISION = 10 ** 18;

    uint256 FEE_DENOMINATOR = 10 ** 10;

    address[N_COINS] internal i_coins;
    address[N_COINS] internal i_underlying_coins;
    ...
    constructor(
        ...
        i_coins = _coins;
        i_underlying_coins = _underlying_coins;
        A = _A;
        fee = _fee;
    ...
    function _stored_rates() internal view returns (uint256[N_COINS] memory result) {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            result[i] *= yERC20ForTest(i_coins[i]).getPricePerFullShare();
        }
    }
