...
export class KnownTokens {
    ...
    public addToken = (tokenMetaData:any) : void => {
        this._tokens.push({
            ...
            expiryTime: tokenMetaData.expiryTime
        });
    }

    public getTokenBySymbol = (symbol: string): Token => {
        const symbolInLowerCaseScore = symbol.toLowerCase();
        const token = this._tokens.find(t => t.symbol.toLowerCase() === symbolInLowerCaseScore);
        if (!token) {
...
export const initKnownTokens = (knownTokensMetadata: TokenMetaData[] = KNOWN_TOKENS_META_DATA) => {
...
export const getKnownTokens = (knownTokensMetadata: TokenMetaData[] = KNOWN_TOKENS_META_DATA): KnownTokens => {
    if (!knownTokens) {
        initKnownTokens(knownTokensMetadata);
    }
    return knownTokens;
};
