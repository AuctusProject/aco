...
interface DispatchProps {
    createSignedOrder: (amount: BigNumber, price: BigNumber, expirationTimeSeconds: BigNumber, side: OrderSide) => Promise<SignedOrder>;
...
class SignOrderStep extends React.Component<Props, State> {
    ...
    private readonly _getSignedOrder = async ({ onLoading, onDone, onError }: any) => {
        ...
        const { amount, price, expirationTimeSeconds, side } = step;
        try {
            const signedOrder = await this.props.createSignedOrder(amount, price, expirationTimeSeconds, side);
            onLoading();
...
const SignOrderStepContainer = connect(
    ...
    (dispatch: any) => {
        return {
            ...
            createSignedOrder: (amount: BigNumber, price: BigNumber, expirationTimeSeconds: BigNumber, side: OrderSide) =>
                dispatch(createSignedOrder(amount, price, expirationTimeSeconds, side)),
        };
    },
)(SignOrderStep);
